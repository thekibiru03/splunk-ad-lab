...
Function script:Set-Environment {
    ...
    # We are removing the failing Get-ADDomain command and hardcoding the values.
    $UsersOU="CN=Users,DC=BLANC,DC=local"
    $UPNSuffix = "@BLANC.local"
    # --- END OF CORRECTION ---
...
ForEach ($RandomUser in $RandomUsers) {
  ...
  New-ADUser @UserProperties -Server localhost -ErrorAction Stop 
